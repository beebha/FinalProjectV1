			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="category" title="${message(code: 'survey.category.label', default: 'Category')}" />
					
						<g:sortableColumn property="dateCreated" title="${message(code: 'survey.dateCreated.label', default: 'Date Created')}" />
					
						<th><g:message code="survey.user.label" default="User" /></th>
				...
				<g:each in="${surveyInstanceList}" status="i" var="surveyInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: surveyInstance, field: "category")}</td>
					
						<td><g:formatDate date="${surveyInstance.dateCreated}" /></td>
					
						<td>${fieldValue(bean: surveyInstance, field: "user")}</td>
